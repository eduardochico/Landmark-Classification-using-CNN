...
def lr_finder(min_lr, max_lr, n_steps, loss, model, data_loaders):
    ...
    torch.save(model.state_dict(), "__weights_backup_lr")
    ...
    # at every iteration
    lr_scheduler = lr_scheduler.ReduceLROnPlateau(optimizer)LambdaLR(optimizer, new_lr)
    ...
    model.load_state_dict(torch.load('__weights_backup_lr'))
